...
async function createFormattedExcel(logs, filePath) {
    ...
    logs.forEach((log) => {
        if (!resourceSummary[log.ResourceID]) {
            resourceSummary[log.ResourceID] = {
        ...
        if (log.Status === "success") {
            resourceSummary[log.ResourceID].SuccessCount += 1;
        } else if (log.Status === "failed") {
            resourceSummary[log.ResourceID].FailedCount += 1;
        }
    });

    worksheet.addRow(["Resource Summary"]);
    worksheet.mergeCells("A1:D1");
    const sumHeaderRow = worksheet.getRow(1);
    ...
    worksheet.mergeCells("E1:Z1");
    const mergedCell = worksheet.getCell("E1");
    mergedCell.value = "";
    mergedCell.fill = {};
    ...
    Object.entries(resourceSummary).forEach(([resourceID, summary]) => {
        ...
        if (summary.FailedCount > summary.SuccessCount) {
            row.eachCell((cell) => {
                cell.fill = {
                    type: "pattern",
                    pattern: "solid",
    ...
    worksheet.addRow([]);
    const logsHeaderRow = worksheet.getRow(worksheet.lastRow.number);
    ...
    logsHeaderRow.alignment = { horizontal: "center", vertical: "middle" };
    ...
    const logHeaderRow = worksheet.addRow();
    worksheet.columns.forEach((col, i) => {
        logHeaderRow.getCell(i + 1).value = col.header;
        logHeaderRow.getCell(i + 1).font = {
            bold: true,
            color: { argb: "FFFFFFFF" },
        };
        logHeaderRow.getCell(i + 1).fill = {
            type: "pattern",
            pattern: "solid",
            fgColor: { argb: "FF4F81BD" },
        };
        logHeaderRow.getCell(i + 1).alignment = {
            horizontal: "center",
            vertical: "middle",
        };
    });
